...
# and '-o' or '--output' flag is for saving image with sclera pixels
...
def getscleraPoint(img, blackCoordinates, eyeTopPointX, eyeBottomPointY):
	'''
	Used to get the coordinates of the sclera 
	...
	if circles is not None:
		...
		for i in circles[0,:1]:
			scleraPoint = [int(eyeTopPointX[0])+ i[0],int(eyeBottomPointY[1]) + i[1]]
	else:
		# if HoughCircles is unable to detect the circle than using eyes points to get sclera points 
		a = 0
...
for imageName in os.listdir(args["image"]):
	...
	# getting sclerapoint for left eye
	# leftEye is the cropped part of face image
	leftEyeCoord, eyeBrowCoord = faceVector[36], faceVector[19]
	leftscleraPoint = getscleraPoint(leftEye, eyeLeftBlackPixels, leftEyeCoord, eyeBrowCoord)

	# getting sclerapoint for right eye
	# rightEye is the cropped part of face image
	rightEyeCoord = faceVector[42]
	rightscleraPoint = getscleraPoint(rightEye, eyeRightBlackPixels, rightEyeCoord, eyeBrowCoord)
	
	# drawing sclera points on image
	cv2.circle(image, tuple(leftscleraPoint), 5, (255, 0, 0), -1)
	cv2.circle(image, tuple(rightscleraPoint), 5, (255, 0, 0), -1)
